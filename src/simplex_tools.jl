...
export Simplex, EFPoint, buildSimplex, envyFreePoint
...
function EFPoint(
    simplex::Simplex, utility_A::UtilityFunction.Utility, utility_B::UtilityFunction.Utility
)::EFPoint
...
function _simplex_points(divisions::Int64)::Vector{Point}
    ...
    for (idx, d) in enumerate(0:divisions)
        points[idx] = Point(d / divisions, counselors[d % 2 + 1], "", 0.0)
    end
...
function _update_simplex_points!(
    ...
    for point in points
        choosed_side, benefit = _choose_side(point, vaccines_fraction, utility_A, utility_B)
...
function _envy_free_points(
    simplex::Simplex, utilityA::UtilityFunction.Utility, utilityB::UtilityFunction.Utility
)::Vector{EFPoint}
...
function _mean_point(
    ...
    benefits = broadcast(
        UtilityFunction.integrate_utility, utilities, [intervals, intervals]
    )
