...
function build_utilities(country::String, setup::Symbol)
...
function _build_utility_steps(country::String, setup::Symbol, counselor::Symbol)
    steps::Vector{Dict} = []

    intervals::Vector{Int64} = COUNTRIES[country]["demographic_intervals"]
    ...
    for i in eachindex(intervals)
        ...
        step = Dict(
            "value" => SETUPS[setup][counselor][i],
...
function _update_normalization_factor!(utility::Utility)::Float64
    ...
    value::Float64 = integrate_smooth_step_function(
        utility.steps, interval_start, interval_end
    )

    return utility.normalization_factor = (1.0 / value)
end
...
function update_utility!(utility::Utility, vaccinated_intervals::Vector{Vector{Float64}})
    ...
    return _update_normalization_factor!(utility)
end

function _update_benefits!(
    utility::Utility, vaccinated_intervals::Vector{Vector{Float64}}
)::Nothing
...
function _vaccinated_population(
    utility_step::Dict, vaccinated_interval::Vector{Float64}
)::Float64
